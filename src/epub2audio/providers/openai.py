"""OpenAI TTS provider adapter stub for epub2audio.

This module defines the :class:`OpenAIProvider` adapter, which will map
:class:`~epub2audio.providers.base.NarrationProvider` onto the OpenAI
text-to-speech API.

**Mapping strategy (per ADR-003, §2 — "no business logic in providers")**:
OpenAI supports natural-language narration instructions passed alongside the
synthesis text.  When implemented, this adapter will derive a concise
instruction sentence from the segment's
...
No OpenAI SDK dependency is introduced; this module uses only stdlib and
...
class OpenAIProvider:
    """Stub OpenAI TTS provider adapter.
    ...
    * Derive a natural-language narration instruction string from mood, pace,
      and intensity (e.g. ``"Speak in a hushed, tense tone, slightly slower
      than normal."``).
    * Set ``ProviderRequest.payload["instructions"]`` to that string.
    * Call the OpenAI TTS API with the text and instructions, returning one
    ...
    def render(
        ...
        """Map a narration segment to an OpenAI-specific provider request.
        ...
        Raises:
            NotImplementedError: Always — the OpenAI adapter is not yet
                implemented.
        """
        raise NotImplementedError(
            "OpenAIProvider.render() is not yet implemented. "
    ...
    def synthesize(self, request: ProviderRequest) -> list[AudioChunk]:
        """Call the OpenAI TTS API and return audio chunks.
        ...
        """
        raise NotImplementedError(
            "OpenAIProvider.synthesize() is not yet implemented. "
            "This stub proves the NarrationProvider interface; "
            "full implementation is planned for a future milestone."
        )
